...
class ROD2021Dataset(Dataset):
    def __init__(self, dataset_cfg: dict, model_cfg: dict, training: bool, root_path: str):
        super().__init__()
        ...
        for seq in seqs:
            ...
            anno_path = os.path.join(
                ...
                'train' if training else 'test', f'{seq}.txt')
            for frame in frames:
        ...
        for seq in self.rads.keys():
            ...
            shift_step = model_cfg['window_step'] if training else model_cfg['window_size'] // 2
...
def collate_fn(batch: list[dict]) -> dict:
    ...
    data['confmap'] = torch.stack(
        [b['confmap'] for b in batch], dim=0)
