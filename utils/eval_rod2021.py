...
from cruw.eval.rod.rod_eval_utils import (
    accumulate,
    compute_ols_dts_gts,
    evaluate_img,
    summarize,
...
olsThrs = np.around(
    np.linspace(0.5, 0.9, int(np.round((0.9 - 0.5) / 0.05) + 1), endpoint=True),
    decimals=2,
)
recThrs = np.around(
    np.linspace(0.0, 1.0, int(np.round((1.0 - 0.0) / 0.01) + 1), endpoint=True),
    decimals=2,
)


def evaluate_rod2021(
    submit_dir: str, truth_dir: str, dataset: CRUW
) -> tuple[float, float]:
    ...
    for seqid, (sub_name, gt_name) in enumerate(zip(sub_names, gt_names)):
        ...
        data_path = os.path.join(
            dataset.data_root, "sequences", "test", gt_names[seqid][:-4]
        )
        ...
        try:
            n_frame = int(
                len(
                    os.listdir(
                        os.path.join(
                            data_path, dataset.sensor_cfg.radar_cfg["chirp_folder"]
                        )
                    )
                )
            ...
            n_frame = np.load(
                os.path.join(
                    data_path, dataset.sensor_cfg.radar_cfg["chirp_folder"] + ".npy"
                )
            ).shape[0]
        ...
        olss_all = {
            (imgId, catId): compute_ols_dts_gts(
                gt_dets, sub_dets, imgId, catId, dataset
            )
        ...
        evalImgs = [
            evaluate_img(
                gt_dets, sub_dets, imgId, catId, olss_all, olsThrs, recThrs, dataset
            )
            for imgId in range(n_frame)
            for catId in range(3)
        ]
